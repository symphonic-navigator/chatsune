...
function AssistantMessageBase({ content, thinking, isStreaming, accentColour, highlighter, isBookmarked, onBookmark, canRegenerate, onRegenerate, status = 'completed', refusalText, timeToFirstTokenMs, tokensPerSecond, generationDurationMs, outputTokens, providerName, modelName, messageId, persona }: AssistantMessageProps) {
  ...
  // prefers the freshly-parsed map when persistedRender produced any pills,
  // because the placeholders embedded in `renderText` carry the effectIds
  // generated by THAT parse. Falling back to `cachedPillContents` would be
  // a key mismatch (live-stream effectIds vs. fresh-parse effectIds),
  // making the rehype plugin drop the placeholders and the pills vanish.
  // The cache stays as a fallback for the legacy / backwards-compat path
  // where the persisted content still carries placeholders (no raw tags
  // found by persistedRender), e.g. older backends without raw_content.
  const pillContents = isStreaming
    ? liveStreamPillContents ?? undefined
    : (persistedRender && persistedRender.pillContents.size > 0
